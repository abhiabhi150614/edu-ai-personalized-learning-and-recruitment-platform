...
@router.post("/recruiter/shortlist/{shortlist_id}/schedule-interview")
def schedule_interview_alt_path(shortlist_id: int, data: Dict[str, Any], credentials: HTTPAuthorizationCredentials = Depends(bearer), db: Session = Depends(get_db)):
    """Alternative endpoint path for scheduling interviews"""
    return schedule_interview_for_shortlisted(shortlist_id, data, credentials, db)
