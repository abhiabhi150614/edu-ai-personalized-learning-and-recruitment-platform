...
class ComposioAuthService:
    """Service for handling LinkedIn and GitHub authentication via Composio"""
    
    def __init__(self):
        self.api_key = os.getenv("COMPOSIO_API_KEY", "ak_nsf-0GU62pD5RCWVXyRN")
    ...
    def create_linkedin_post(self, user_email: str, content: str, visibility: str = "PUBLIC") -> Dict[str, Any]:
        ...
        DEFAULT_CREDENTIALS = {
            "author_id": "urn:li:person:F-GGHi_PH4",
            "profile_name": "Alars Felar",
            "email": "alarsfelar@gmail.com"
        }
        
        try:
            unique_id = self._get_unique_user_id(user_email)
            print("\n" + "="*80)
            print("[LINKEDIN POST] STARTING POST CREATION")
            print("="*80)
            print(f"User Email: {user_email}")
            print(f"Composio User ID: {unique_id}")
            print(f"Post Content: {content}")
            print(f"Visibility: {visibility}")
            print("="*80)
            
            # First get LinkedIn profile to get correct author_id
            print("\n[LINKEDIN POST] Fetching LinkedIn profile...")
            ...
            if "profile" not in profile_result:
                print("[LINKEDIN POST] ⚠️ Profile fetch failed, using DEFAULT credentials")
                author_id = DEFAULT_CREDENTIALS["author_id"]
                profile_name = DEFAULT_CREDENTIALS["profile_name"]
                using_default = True
            else:
                # Extract and display full profile data
                profile_data = profile_result["profile"]
                print("\n" + "-"*80)
                print("[LINKEDIN PROFILE] FULL PROFILE DATA:")
                print("-"*80)
                import json
                ...
                if isinstance(profile_data, dict):
                    if "data" in profile_data and "response_dict" in profile_data["data"]:
                        response_dict = profile_data["data"]["response_dict"]
                        author_id = response_dict.get("author_id")
                        profile_name = response_dict.get("name", "N/A")
                        profile_headline = response_dict.get("headline", "N/A")
                        profile_url = response_dict.get("profile_url", "N/A")
                    elif "response_dict" in profile_data:
                        response_dict = profile_data["response_dict"]
                        author_id = response_dict.get("author_id")
                        profile_name = response_dict.get("name", "N/A")
                        profile_headline = response_dict.get("headline", "N/A")
                        profile_url = response_dict.get("profile_url", "N/A")
            ...
            if not author_id:
                print("\n[LINKEDIN POST] ⚠️ Could not extract author_id, using DEFAULT credentials")
                author_id = DEFAULT_CREDENTIALS["author_id"]
                profile_name = DEFAULT_CREDENTIALS["profile_name"]
                using_default = True
            
            print("\n" + "-"*80)
            print("[LINKEDIN CREDENTIALS] EXTRACTED PROFILE INFO:")
            print("-"*80)
            ...
            if using_default:
                print(f"⚠️ Using DEFAULT credentials (fallback)")
            print("-"*80)
            ...
            print("\n" + "-"*80)
            print("[LINKEDIN POST] POST PARAMETERS:")
            print("-"*80)
            print(json.dumps(post_params, indent=2))
            print("-"*80)
            
            print("\n[LINKEDIN POST] Executing post creation...")
            result = self.composio.tools.execute(
            ...
            print("\n" + "-"*80)
            print("[LINKEDIN POST] API RESPONSE:")
            print("-"*80)
            ...
            if result.get('successful') and result.get('data', {}).get('response_data', {}).get('status_code') == 201:
                ...
                print("\n" + "="*80)
                print("[LINKEDIN POST] ✅ POST CREATED SUCCESSFULLY!")
                print("="*80)
                print(f"Share ID: {share_id}")
                print(f"Posted by: {profile_name}")
                print(f"Profile: {profile_url}")
                if using_default:
                    print(f"⚠️ Posted using DEFAULT credentials")
                ...
                print("\n" + "="*80)
                print("[LINKEDIN POST] ❌ POST CREATION FAILED")
                print("="*80)
            ...
            print("\n" + "="*80)
            print("[LINKEDIN POST] ❌ EXCEPTION OCCURRED")
            print("="*80)
            print(f"Error: {str(e)}")
            print("="*80 + "\n")
            import traceback
